...
def load_image_from_path(img_path):
    ...
    img = tf.keras.layers.Resizing(299, 299)(img)  # Resize image to desired dimensions
    img = tf.cast(img, tf.float32)  # Convert the image to float32
...
def get_caption_model():
    ...
    caption_model.load_weights('new(2oct morn).weights.h5')
...
def play_audio(audio_file_path):
    ...
    try:
        ...
        pygame.mixer.init()
        pygame.mixer.music.load(audio_file_path)  # Load the audio file
        pygame.mixer.music.play()  # Play the audio file

        # Wait for the music to finish playing
        while pygame.mixer.music.get_busy():
            pygame.time.Clock().tick(10)  # Wait a bit to avoid busy-waiting
    except Exception as e:
...
def generate_audio(caption):
    ...
    # Create a timestamp for the audio file
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    audio_file_path = os.path.join('captions', f'caption_{timestamp}.mp3')

    # Generate audio using gTTS
    tts = gTTS(text=caption, lang='en')
    tts.save(audio_file_path)

    print(f"Audio saved at: {audio_file_path}")  # Confirm audio is saved
...
def generate_caption(img, caption_model):
    try:
        ...
        audio_file_path = generate_audio(y_inp)
